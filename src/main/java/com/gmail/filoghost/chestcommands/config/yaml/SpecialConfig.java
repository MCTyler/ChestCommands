...
public class SpecialConfig {
	...
	private final transient PluginConfig config;
			...
			for (Field field : getClass().getDeclaredFields()) {
				...
				try {
					...
					if (defaultValue != null) {
						...
						config.getPlugin().getLogger().log(Level.WARNING, "The field {0} was not provided with a default value, please inform the developer.", field.getName());
					}
		...
		for (Field field : getClass().getDeclaredFields()) {
			...
			if (config.isSet(configKey)) {
				...
				if (type == boolean.class || type == Boolean.class) {
					...
					config.getPlugin().getLogger().log(Level.WARNING, "Unknown field type: {0} ({1}). Please inform the developer.", new Object[]{field.getType().getName(), field.getName()});
				}
